...
%   delta_C     the fractional deviation from the expected clustering coefficient of a
%                   random network
...
symcheck=abs(A-A');
symcheck(find(symcheck<0.000001))=0;
...
rand_path = avg_path_matrix(1./W_rand);    %path of the random network
net_path = avg_path_matrix(1./W);          %path of the network

% reg_path = mean(mean(distance_wei(1./W_reg)));      %path of the regular network
% rand_path = mean(mean(distance_wei(1./W_rand)));    %path of the random network
% net_path = mean(mean(distance_wei(1./W)));          %path of the input network
...
%XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
%XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
%the above code requires the following functions
...
M2 = M;
M2(find(M<0)) = 1+M2(find(M<0));
M2(find(M>0)) = 1-M2(find(M>0));
D = distance_wei(M2);

% [J_st,route_st,J,route]=shortestPathDP(L,s,t,steps);
